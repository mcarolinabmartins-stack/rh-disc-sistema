...
import { ESTADOS_BR, NIVEIS, REGIMES, SETORES_SUGERIDOS, formatCurrency } from "@/lib/utils";
...
export default function ColaboradoresListPage() {
  ...
  async function load() {
    setLoading(true);
    const [{ data: colabs }, { data: cargosData }] = await Promise.all([
      supabase.from("colaboradores").select("*, cargo:cargos(*)").order("nome"),
  ...
  return (
    <div>
      ...
      {showNew && <NewColaboradorModal cargos={cargos} onClose={() => setShowNew(false)} onCreated={load} />}
    </div>
  );
}

function NewColaboradorModal({ cargos, onClose, onCreated }: { cargos: Cargo[]; onClose: () => void; onCreated: () => void }) {
